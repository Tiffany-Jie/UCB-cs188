    ...
    def train(self, dataset):
        ...
        while change:
            ...
            for x, y in dataset.iterate_once(batch_size):
                predictY = self.get_prediction(x)
                if predictY != nn.as_scalar(y):
                    direction = x
    ...
    def train(self, dataset):
        ...
        while loss > 0.015: 
            for x, y in dataset.iterate_once(batch_size):
                ...
                #print("loss:",loss)
                gradients = nn.gradients(loss_node, self.parameters)
                for i in range(len(self.parameters)):
                    self.parameters[i].update(gradients[i], -learning_rate)
